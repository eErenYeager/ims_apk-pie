...
public final class ImsSenderRxr extends ImsPhoneBaseCommands implements ImsPhoneCommandsInterface {
    ...
    private RegistrantList mVoWiFiCallQualityRegistrants = new RegistrantList();
    private RegistrantList mSsaccRegistrants = new RegistrantList();
    ...
    public void registerForPhoneId(int phoneId) {
        ...
        if (mSocket != null) {
            try {
                ...
                Log.e(this, "registerForPhoneId: " + mInstanceId + "' socket closed",
                                    ex);
            }
        }
    }
            ...
            switch (msg.what) {
                case EVENT_SEND:
                    ...
                    try {
                        ...
                        Log.e(this, "Uncaught exception ", exc);
                        req = findAndRemoveRequestFromList(rr.mSerial);
                        // make sure this request has not already been handled,
                        // eg, if IFMsg_Rxr cleared the list.
                        if (req != null || !alreadySubtracted) {
                    ...
                    synchronized (mWakeLock) {
                        if (mWakeLock.isHeld()) {
                            ...
                            if (mRequestMessagesWaiting != 0) {
                                Log.i(this, "NOTE: mReqWaiting is NOT 0 but"
                            ...
                            // occurs.
                            if (mRequestMessagesPending != 0) {
                                Log.e(this, "ERROR: mReqPending is NOT 0 but"
    ...
    class IFMsg_Rxr implements Runnable {
        ...
        public void run() {
            ...
            try {
                if (!killMe) {
                    for (;;) {
                        ...
                        try {
                             ...
                             s.connect(l);
                             Log.i(this, "Connecting to socket " + s);
                        ...
                        Log.i(this, "Connected to '" + mSocket + "' socket");
                        Log.i(this, "For instance [" + mInstanceId + "] connected to '" + ifSocket + "' socket");
                        ...
                        try {
                            ...
                            for (;;) {
                                ...
                                Log.v(this, "Read packet: " + length + " bytes");

                                if (length > 0) processResponse(buffer, length);

                            }
                        } catch (java.io.IOException ex) {
                            Log.i(this, "'" + ifSocket + "' socket closed" + ex);
                        } catch (Throwable tr) {
                            Log.e(this, "Uncaught exception read length=" + length +
                                    "Exception:" + tr.toString());
                        }

                        Log.i(this, "Disconnected from " + ifSocket + " socket");
                ...
                Log.e(this, "Uncaught exception" + tr);
            }
    ...
    public void send(IFRequest rr) {
        ...
        if (mSocket == null) {
            Log.e(this, "socket connection with ril is null");
            rr.onError(ImsQmiIF.E_RADIO_NOT_AVAILABLE, null);
            rr.release();
            return;
        }
    ...
    protected void processSolicited(ImsQmiIF.MsgTag tag, byte[] message) {
        ...
        if (rr == null) {
            Log.w(this, "Unexpected solicited response! sn: "
                    + serial + " error: " + error);
            return;
        }
            ...
            try {
                switch (id) {
                    ...
                    case ImsQmiIF.REQUEST_GET_VOPS_INFO:
                        ret = handleVopsInfo(message);
                        break;
                    case ImsQmiIF.REQUEST_GET_SSAC_INFO:
                        ret = handleSsacInfo(message);
                        break;
                    case ImsQmiIF.REQUEST_GET_VOLTE_PREF:
                        ret = responseGetVoltePref(message);
                        break;
                    case ImsQmiIF.REQUEST_GET_HANDOVER_CONFIG:
                        ret = responseGetHanoverConfig(message);
                        break;
                    ...
                    case ImsQmiIF.REQUEST_ADD_PARTICIPANT:
                        ret = responseSipErrorInfo(message);
                        break;
    ...
    protected void processUnsolicited(ImsQmiIF.MsgTag tag, byte[] message) {
        ...
        try {
            switch (response) {
                ...
                case ImsQmiIF.UNSOL_REQUEST_GEOLOCATION:
                    if (message != null) ret = handleGeolocation(message);
                    break;
                case ImsQmiIF.UNSOL_REFRESH_VICE_INFO:
                    if (message != null) ret = handleRefreshViceInfo(message);
                    break;
                case ImsQmiIF.UNSOL_VOWIFI_CALL_QUALITY:
                    if (message != null) ret = handleVoWiFiCallQuality(message);
                    break;
                case ImsQmiIF.UNSOL_VOPS_CHANGED:
                    if (message != null) ret = handleVopsInfo(message);
                    break;
                case ImsQmiIF.UNSOL_SSAC_CHANGED:
                    if (message != null) ret = handleSsacInfo(message);
                    break;
                case ImsQmiIF.UNSOL_PARTICIPANT_STATUS_INFO:
                    if (message != null) ret = handleParticipantStatusInfo(message);
                    break;
                default:
                    throw new RuntimeException("Unrecognized unsol response: " + response);
            }
        } catch (Throwable tr) {
            Log.e(this, "Exception processing unsol response: " + response +
                    "Exception:" + tr.toString());
            return;
        }
        ...
        switch (response) {
            ...
            case ImsQmiIF.UNSOL_REQUEST_GEOLOCATION:
                unsljLogRet(response, ret);
                if (ret != null) {
                    mGeolocationRegistrants.notifyRegistrants(
                            new AsyncResult (null, ret, null));
                }
                break;
            case ImsQmiIF.UNSOL_REFRESH_VICE_INFO:
    ...
    private Object responseGetHanoverConfig(byte[] message) {
        ImsQmiIF.HandoverConfigMsg handoverConfigMsg = null;
        if (message != null && message.length >= 1) {
            try {
                handoverConfigMsg = ImsQmiIF.HandoverConfigMsg.parseFrom(message);
                if (handoverConfigMsg.hasHoConfig()) {
                    int hoConfig = handoverConfigMsg.getHoConfig();
                    log("responseGetHanoverConfig, hoConfig = " + hoConfig);
                    return hoConfig;
                } else {
                    log("responseGetHanoverConfig failed");
                }
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log(" Message parsing. InvalidProtocolBufferException ");
            }
        } else {
            log("responseGetHanoverConfig failed, message is null");
        }
        return handoverConfigMsg;
    }

    private Object responseSipErrorInfo(byte[] message) {
        ImsQmiIF.SipErrorInfo sipErrorInfo = null;
        if (message != null && message.length > 0) {
            try {
                sipErrorInfo = ImsQmiIF.SipErrorInfo.parseFrom(message);
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log(" Message parsing. InvalidProtocolBufferException ");
            }
        } else {
            log("responseSipErrorInfo failed, message is null");
        }
        return sipErrorInfo;
    }
    ...
    public void log(String msg) {
        Log.i(this, msg + "[SUB" + mInstanceId + "]");
    }

    public void logv(String msg) {
        Log.v(this, msg + "[SUB" + mInstanceId + "]");
    }
    ...
    private byte[] processDial(String address, int clirMode,
        ...
        if (callDetails != null) {
            isCallPull = callDetails.getCallPull();
        }

        logv("process dial isConfererenceUri = " + isConferenceUri + ",isCallPull = " + isCallPull);
        ...
        if (isCallPull) {
            dialIF.setIsCallPull(isCallPull);
        }

        dialIF.setIsEncrypted(isEncrypted);
    ...
    public void addParticipant(String address, int clirMode, CallDetails callDetails,
        ...
        byte[] dialb = processDial(address, clirMode, callDetails, false);
    ...
    dial(String address, int clirMode, CallDetails callDetails,
        ...
        byte[] dialb = processDial(address, clirMode, callDetails, isEncrypted);
    ...
    public synchronized void sendGeolocationInfo(double lat, double lon,
        ...
        StringBuilder sb = new StringBuilder("sendGeolocationInfo [Lat="
                + lat + " | Lon=" + lon + "]");
        ImsQmiIF.AddressInfo addressInfo = new ImsQmiIF.AddressInfo();
        if (address != null) {
            info = address.getLocality();
            if (info != null) {
                addressInfo.setCity(info);
            ...
            if (info != null) {
                addressInfo.setPostalCode(info);
                sb.append(" Postal Code=" + info);
            }
            info = address.getCountryCode();
            if (info != null) {
                addressInfo.setCountryCode(info);
        ...
        geoInfo.setLat(lat);
        geoInfo.setLon(lon);
        geoInfo.setAddressInfo(addressInfo);
        byte[] geoInfob = geoInfo.toByteArray();

        encodeMsg(ImsQmiIF.REQUEST_SEND_GEOLOCATION_INFO, result, geoInfob);
    }
    ...
    public void explicitCallTransfer(int srcCallId, int type, String number,
        ...
        ImsQmiIF.ExplicitCallTransfer ectMsg = new ImsQmiIF.ExplicitCallTransfer();
        ectMsg.setCallId(srcCallId);
        ectMsg.setEctType(type);
        if (number != null) ectMsg.setTargetAddress(number);
        if (destCallId > 0) ectMsg.setTargetCallId(destCallId);
        byte[] ectb = ectMsg.toByteArray();
        encodeMsg(ImsQmiIF.REQUEST_EXPLICIT_CALL_TRANSFER, result, ectb);
    }
    ...
    public void sendConfigRequest(int requestType, int item, boolean boolValue,
        ...
        ImsQmiIF.ConfigMsg configMsg = new ImsQmiIF.ConfigMsg();
        configMsg.setItem(item);
        configMsg.setBoolValue(boolValue);
        configMsg.setIntValue(intValue);
        if (strValue != null) configMsg.setStringValue(strValue);
        configMsg.setErrorCause(errorCause);
        byte[] configb = configMsg.toByteArray();

        encodeMsg(requestType, result, configb);
    }
    ...
    protected Object responseImsRegistration(byte[] imsRegB) {
        ...
        if (imsRegB != null && imsRegB.length >= 1) {
            ...
            Log.e(this, "responseImsRegistration error");
        }
        return registration;
    }
    ...
    protected Object responseQueryCallForward(byte[] callInfoList) {
        ...
        ImsCallForwardTimerInfo timerInfo[] = null;
        int numInfos = 0;
        ...
        if (callInfoList != null) {
            try {
                ...
                timerInfo = new ImsCallForwardTimerInfo[numInfos];
                for (int i = 0; i < numInfos; i++) {
                    ...
                    if (callInfo.hasCallFwdTimerStart() && callInfo.hasCallFwdTimerEnd()) {
                        ...
                        timerInfo[i].status = callInfo.getStatus();
                        timerInfo[i].reason = callInfo.getReason();
                        timerInfo[i].serviceClass = callInfo.getServiceClass();
                        timerInfo[i].number = callInfo.getNumber();
                        timerInfo[i].startHour = startCallTimerInfo.getHour();
                        timerInfo[i].startMinute = startCallTimerInfo.getMinute();
                        ImsQmiIF.CallFwdTimerInfo endCallTimerInfo =
                                callInfo.getCallFwdTimerEnd();
                        timerInfo[i].endHour = endCallTimerInfo.getHour();
                        timerInfo[i].endMinute = endCallTimerInfo.getMinute();
                        return timerInfo;
                    }
                }
    ...
    protected Object responseQueryClip(byte[] clipInfo) {
        if (clipInfo != null) {
            try {
                return ImsQmiIF.ClipProvisionStatus.parseFrom(clipInfo);
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log("InvalidProtocolBufferException in message tag parsing");
            }
        }
    ...
    protected Object responseGetColr(byte[] colrInfo) {
        ImsQmiIF.Colr info = null;

        if (colrInfo != null) {
            try {
                info = ImsQmiIF.Colr.parseFrom(colrInfo);
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log("InvalidProtocolBufferException in message tag parsing");
            }
        }
    ...
    private Object responseConfig(byte[] response) {
        ImsQmiIF.ConfigMsg config = null;
         try {
             config = ImsQmiIF.ConfigMsg.parseFrom(response);
             if (config.hasBoolValue()) {
                 return config.getBoolValue();
             } else if (config.hasIntValue()) {
                 return config.getIntValue();
             } else if (config.hasStringValue()) {
                 return config.getStringValue();
             } else if (config.hasErrorCause()) {
                 return config.getErrorCause();
             }
         } catch (NullPointerException e) {
             log("NullPointerException in responseConfig");
         } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
             log("InvalidProtocolBufferException in responseHandover parsing");
         }
         return config;
            ...
            try {
                ...
                switch (state.getState()) {
                    ...
                    case ImsQmiIF.RADIO_STATE_ON:
                        ...
                        break;
                    default:
                        Log.e(this, "Invalid state in Radio State Change");
                        break;
    ...
    protected Object handleGeolocation(byte[] geolocationRequest) {
        ImsQmiIF.GeoLocationInfo geoInfo = null;
        if (geolocationRequest != null && geolocationRequest.length >= 1) {
            try {
                geoInfo = ImsQmiIF.GeoLocationInfo.parseFrom(geolocationRequest);
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log("InvalidProtocolBufferException in handleGeolocation parsing");
            }
        }
        return geoInfo;
    }
    ...
    protected Object handleRefreshViceInfo(byte[] viceInfo) {
        ImsQmiIF.ViceInfo info = null;
        if (viceInfo != null && viceInfo.length >= 1) {
            try {
                info = ImsQmiIF.ViceInfo.parseFrom(viceInfo);
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log("InvalidProtocolBufferException in responseHandover parsing");
            }
        }
    ...
    protected Object handleVoWiFiCallQuality(byte[] voWiFiCallQuality) {
        int[] response = null;
        if (voWiFiCallQuality != null) {
            try {
                ImsQmiIF.VoWiFiCallQuality callQuality = null;
                callQuality = ImsQmiIF.VoWiFiCallQuality.parseFrom(voWiFiCallQuality);
                response = new int[1];
                response[0] = callQuality.getQuality();
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log("InvalidProtocolBufferException in handleVoWiFiCallQuality parsing");
            }
        }
        return response;
    }

    protected Object handleParticipantStatusInfo(byte[] participantStatusInfo) {
        ImsQmiIF.ParticipantStatusInfo info = null;
        if (participantStatusInfo != null && participantStatusInfo.length >= 1) {
            try {
                info = ImsQmiIF.ParticipantStatusInfo.parseFrom(participantStatusInfo);
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log("InvalidProtocolBufferException in responseHandover parsing");
            }
        }
    ...
    private Object handleVopsInfo(byte[] vopsInfo) {
        if (vopsInfo != null && vopsInfo.length >= 1) {
            try {
                return ImsQmiIF.VopsInfo.parseFrom(vopsInfo);
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log("InvalidProtocolBufferException in handleVopsInfo parsing");
            }
        }
        return null;
    }

    private Object handleSsacInfo(byte[] ssacInfo) {
        if (ssacInfo != null && ssacInfo.length >= 1) {
            try {
                return ImsQmiIF.SsacInfo.parseFrom(ssacInfo);
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log("InvalidProtocolBufferException in handleSsacInfo parsing");
            }
        }
        return null;
    }

    protected Object responseGetVoltePref(byte[] voltePref) {
        int[] response = null;
        if (voltePref != null && voltePref.length >= 1) {
            try {
                ImsQmiIF.VolteUserPref pref = ImsQmiIF.VolteUserPref.parseFrom(voltePref);
                response = new int[1];
                response[0] = pref.getMode();
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log("InvalidProtocolBufferException in handleVoWiFiCallQuality parsing");
            }
        }
    ...
    protected Object handleTtyNotify(byte[] notification) {
        ...
        if (notification != null) {
            try {
                ...
                Log.i(this, "handleTtyNotify mode = " + mode[0]);
            } catch (com.google.protobuf.micro.InvalidProtocolBufferMicroException ex) {
                log("InvalidProtocolBufferException in Message Tag parsing ");
            }
        }
            ...
            try {
                ...
                for (int i = 0; i < num; i++) {
                    ...
                    if (call.hasIsEncrypted()) {
                        dc.isEncrypted = call.getIsEncrypted();
                    } else {
                        dc.isEncrypted = false;
                    }
                    if (call.hasHistoryInfo()) {
                        dc.historyInfo = call.getHistoryInfo();
                    }
                    ...
                    dc.callDetails.callsubstate = toCallSubstateConstants(
                            call.getCallDetails().getCallSubstate());
                    ...
                    dc.callDetails.causeCode = call.getCallDetails().getCauseCode();
                    Log.v(this, "Call Details = " + dc.callDetails);
                    ...
                    if (call.hasFailCause()) {
                        ...
                        if (call.getFailCause().hasErrorDetails()) {
                            networkError = call.getFailCause().getErrorDetails()
                                    .getSipErrorString();
                        ...
                        if (call.getFailCause().hasFailcause()) {
                            ...
                            if (call.getFailCause().hasErrorDetails()) {
                                dc.callFailCause.mExtraCode
                                    = call.getFailCause().getErrorDetails().getSipErrorCode();
                            }
                            // Get call fail reason
                            dc.mCallFailReason = call.getFailCause().getFailcause();
    ...
    private int getImsReasonForCallFailCause(ImsQmiIF.CallFailCauseResponse failCauseResp) {
        ...
        switch (failCause) {
            ...
            case ImsQmiIF.CALL_FAIL_SIP_SERVER_INTERNAL_ERROR:
                //TODO - Compilation stub imsCode = ImsReasonInfo.CODE_SIP_SERVER_INTERNAL_ERROR;
                break;
            case ImsQmiIF.CALL_FAIL_SIP_SERVER_NOT_IMPLEMENTED:
                //TODO - Compilation stub imsCode = ImsReasonInfo.CODE_SIP_SERVER_NOT_IMPLEMENTED;
                break;
            case ImsQmiIF.CALL_FAIL_SIP_SERVER_BAD_GATEWAY:
                //TODO - Compilation stub imsCode = ImsReasonInfo.CODE_SIP_SERVER_BAD_GATEWAY;
                break;
            ...
            case ImsQmiIF.CALL_FAIL_SIP_SERVER_VERSION_UNSUPPORTED:
                //TODO - Compilation stub imsCode = ImsReasonInfo.CODE_SIP_SERVER_VERSION_UNSUPPORTED;
                break;
            case ImsQmiIF.CALL_FAIL_SIP_SERVER_MESSAGE_TOOLARGE:
                //TODO - Compilation stub imsCode = ImsReasonInfo.CODE_SIP_SERVER_MESSAGE_TOOLARGE;
                break;
            case ImsQmiIF.CALL_FAIL_SIP_SERVER_PRECONDITION_FAILURE:
                //TODO - Compilation stub imsCode = ImsReasonInfo.CODE_SIP_SERVER_PRECONDITION_FAILURE;
                break;
            ...
            case ImsQmiIF.CALL_FAIL_FDN_BLOCKED:
                ...
                break;
            case ImsQmiIF.CALL_FAIL_CS_RETRY_REQUIRED:
                imsCode = ImsReasonInfo.CODE_LOCAL_CALL_CS_RETRY_REQUIRED;
                break;
            case ImsQmiIF.CALL_FAIL_HO_NOT_FEASIBLE:
                
                break;
            case ImsQmiIF.CALL_FAIL_LOW_BATTERY:
                imsCode = ImsReasonInfo.CODE_LOW_BATTERY;
                break;
            ...
            case ImsQmiIF.CALL_FAIL_EMERGENCY_PERM_FAILURE:
                ...
                break;
            case ImsQmiIF.CALL_FAIL_PULL_OUT_OF_SYNC:
                imsCode = ImsReasonInfo.CODE_CALL_PULL_OUT_OF_SYNC;
                break;
            case ImsQmiIF.CALL_FAIL_CAUSE_CALL_PULLED:
    ...
    public void setUiTTYMode(int uiTtyMode, Message response) {
        Log.i(this, "setUiTTYMode uittyMode=" + uiTtyMode);
...
/*
    ...
    public void setWifiCallingPreference(int wifiCallingStatus, int wifiCallingPreference,
            Message response) {
        Log.i(this, "setWifiCallingPreference wifiCallingStauts : " + wifiCallingStatus +
    ...
    public void getHandoverConfig(Message response) {
        Log.d(LOG_TAG, "getHandoverConfig");
        encodeMsg(ImsQmiIF.REQUEST_GET_HANDOVER_CONFIG, response, null);
    }

    public void setHandoverConfig(int hoConfig, Message response) {
        Log.d(LOG_TAG, "setHandoverConfig hoConfig: " + hoConfig);
        ImsQmiIF.HandoverConfigMsg hoConfigMsg = new ImsQmiIF.HandoverConfigMsg();
        hoConfigMsg.setHoConfig(hoConfig);
        encodeMsg(ImsQmiIF.REQUEST_SET_HANDOVER_CONFIG, response,
                hoConfigMsg.toByteArray());
    }
    ...
    public void queryVopsStatus(Message response) {
        ...
        encodeMsg(ImsQmiIF.REQUEST_GET_VOPS_INFO, response, null);
    }

    public void querySsacStatus(Message response) {
        Log.i(this, "querySsacStatus");
        encodeMsg(ImsQmiIF.REQUEST_GET_SSAC_INFO, response, null);
    }

    public void updateVoltePref(int preference, Message response) {
        Log.i(this, "updateVoltePref as " + preference);
        ImsQmiIF.VolteUserPref prefMsg = new ImsQmiIF.VolteUserPref();
        prefMsg.setMode(preference);
        encodeMsg(ImsQmiIF.REQUEST_SET_VOLTE_PREF, response, prefMsg.toByteArray());
    }

    public void queryVoltePref(Message response) {
        Log.i(this, "queryVoltePref");
        encodeMsg(ImsQmiIF.REQUEST_GET_VOLTE_PREF, response, null);
    }
}
